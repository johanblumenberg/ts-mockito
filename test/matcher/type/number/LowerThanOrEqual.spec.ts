...
import {LowerThanOrEqualMatcher} from "../../../../src/matcher/type/number/LowerThanOrEqualMatcher";
import {imock, instance, lowerThanOrEqual, when} from "../../../../src/ts-mockito";

describe("LowerThanOrEqualMatcher", () => {
    describe("lowerThanOrEqual()", () => {
        it("should be a IfAfterOrEqualMatcher instance", () => {
            expect((lowerThanOrEqual(0) as any) instanceof LowerThanOrEqualMatcher).toBe(true);
        });

        it("should be a number type", () => {
            expectType<number>(lowerThanOrEqual(0));
        });
    });

    describe("#match()", () => {
        it("should return true when the given number is lower than the expected number", () => {
            const matcher = new LowerThanOrEqualMatcher(0);
    ...
    describe("#toString()", () => {
        it("should correctly get the string representation", () => {
            ...
            expect(new LowerThanOrEqualMatcher(expected).toString()).toBe(`lowerThanOrEqual(${expected})`);
        });
    });
    ...
    describe("stubbing a method", () => {
        ...
        it("should pass the verification when the given number is lower than the expected number", () => {
            ...
            when(testServiceMock.testMethod(lowerThanOrEqual(expected))).thenReturn(true);
